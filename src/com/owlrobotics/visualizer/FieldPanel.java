...
public class FieldPanel extends JPanel {
    ...
    public ArrayList<BotEntity> entities;
    public ArrayList<PathChain> chain = new ArrayList<>();
    ...
    // The JFrame will be set to the frame created in PathVisualizer, it is required to get the frames location
    JFrame frame;

    // Plane Origin set in PathVisualizer
    public PlaneOrigin origin;
    ...
    // FieldPanel class constructor
    public FieldPanel(BufferedImage fieldImg, JFrame frame, ArrayList<BotEntity> entities, double pixelsPerInch, PlaneOrigin origin, double fieldRotation, int fieldSize, int targetFPS) {
        // Set variables defined above
        this.fieldImg = fieldImg.getScaledInstance(fieldSize, fieldSize, Image.SCALE_SMOOTH);
        this.frame = frame;
        ...
        xOrigin = origin.getOrigin(fieldSize).width;
        yOrigin = origin.getOrigin(fieldSize).height - 10;

        // Set the Panel size based on the fieldImage size
        Dimension size = new Dimension(fieldSize, fieldSize);
        this.setPreferredSize(size);

        // Set the layoutManager to XYLayout
        this.setLayout(new XYLayout());

        // Set the panel location on the MainPanel
        this.setLocation(20, 20);

        initAll();

        // Set the FieldPanel to be focusable and visible
    ...
    public void initAll() {
        ...
        for (int botNumber = 0; botNumber < entities.size(); botNumber++) {
            ...
            for (int pathNumber = 0; pathNumber < chain.get(botNumber).size(); pathNumber ++) {
                ...
                if (chain.get(botNumber).getPath(pathNumber).getPathColor() == null) {
                ...
                for (; pointNumber < chain.get(botNumber).getPath(pathNumber).getControlPoints().size(); pointNumber++) {
                    ...
                    buttons.get(botNumber)[pointNumber + nextPath - minusPath] = new ControlButton(
                            new Dimension(16, 16),
                            (int) (chain.get(botNumber).getPath(pathNumber).getControlPoints().get(pointNumber).getX() * pixelsPerInch) - 6 + xOrigin,
                            (int) -(chain.get(botNumber).getPath(pathNumber).getControlPoints().get(pointNumber).getY() * pixelsPerInch) + 2 + yOrigin,
                            controlPointLocations.get(botNumber)[pointNumber + nextPath - minusPath],
                            chain.get(botNumber).getPath(pathNumber),
                            pointNumber, chain.get(botNumber).getPath(pathNumber).getControlPoints().size()
                    );

                    // Add each button to the FieldPanel
    ...
    class ControlButton extends JPanel {
        ...
        public ControlButton(Dimension size, int x, int y, Point point, Path path, int pointNumber, int pathSize) {
            this.path = path;
            ...
            this.addMouseMotionListener(new Listener(this, point));
        ...
        public void paintComponent(Graphics g) {
            ...
            if (pointNumber != 0 && pointNumber != pathSize - 1) {
                g2d.setColor(Color.WHITE);
                g2d.drawString("" + pointNumber, 4, getHeight() - 4);
            }
        }
    }

    // This is the main loop of the Visualizer
    // Everything in FieldPanel is drawn here and the fps limiter si implemented here
    @Override
    public void paintComponent(Graphics g) {
    ...
    public void setOrigin(PlaneOrigin origin) {
        this.origin = origin;
        xOrigin = origin.getOrigin(fieldSize).width;
        yOrigin = origin.getOrigin(fieldSize).height - 10;
    }
    ...
    public void DrawBezierCurves(Graphics2D g2d) {
        ...
        for (int botNumber = 0; botNumber < entities.size(); botNumber++) {
            ...
            for (int pathNumber = 0; pathNumber < chain.get(botNumber).size(); pathNumber++) {
                ...
                if (pathNumber == 0) {
                    ...
                    for (int pointNumber = 0; pointNumber < chain.get(botNumber).getPath(pathNumber).getControlPoints().size(); pointNumber++) {
                        ...
                        buttons.get(botNumber)[pointNumber + nextPath].setLocation(
                                (int) (chain.get(botNumber).getPath(pathNumber).getControlPoints().get(pointNumber).getX() * pixelsPerInch) - 7 + xOrigin,
                                (int) -(chain.get(botNumber).getPath(pathNumber).getControlPoints().get(pointNumber).getY() * pixelsPerInch) + 3 + yOrigin);
                    }
                ...
                chain.get(botNumber).getPath(pathNumber).init();

                // Get DashboardDrawingPoints
                chain.get(botNumber).getPath(pathNumber).init();
                curvePoints.get(botNumber).set(pathNumber, chain.get(botNumber).getPath(pathNumber).getPanelsDrawingPoints());
    ...
    class Listener implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent e) {
            ...
            panel.setLocation(
                    e.getXOnScreen() - fieldLocationX - 7,
                    e.getYOnScreen() - fieldLocationY - 40 + 3);

            // Set the point location
            point.setCoordinates(
                    -(xOrigin - (e.getXOnScreen() -  fieldLocationX)) / pixelsPerInch,
    ...
    class MListener implements MouseListener {
        ...
        // Set doUpdate to true for ControlPanel
        @Override
        public void mousePressed(MouseEvent e) {
            doUpdate = true;
        }

        // Set doUpdate to false for ControlPanel
        @Override
    ...
    class RunAnimation extends Thread {
        ...
        int distanceToNextBotLocationX;
        int distanceToNextBotLocationY;
        int distanceToNextBotLocation;
        ...
        public void run() {
            ...
            // Create integer arrays for distanceToNextBotLocation
            distanceToNextBotLocationX = 0;
            distanceToNextBotLocationY = 0;
            distanceToNextBotLocation = 0;
            ...
            while (runAnimation || runSingleBotAnimation[botNumber]) {
                ...
                if (animate1[botNumber] < 100) {
                    ...
                    distanceToNextBotLocation = (int) Math.sqrt(
                            (distanceToNextBotLocationX * distanceToNextBotLocationX) +
                                    (distanceToNextBotLocationY * distanceToNextBotLocationY));
                ...
                try {
                    sleep(distanceToNextBotLocation * 5L);
